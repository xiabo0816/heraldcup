...
/
├── /matches
│   ├── 页面级范围条：全部 / 先锋杯 / 传奇杯 / 冠绝杯（页内切换）
│   ├── 条件性范围 Banner：仅位于页面级范围条下方，先锋杯 / 传奇杯 / 冠绝杯态出现
│   ├── /matches/[slug]
...
- 顶栏对象入口直达比赛、选手、战队
- 顶栏工具入口直达搜索、我的、后台
- 比赛、选手、战队 3 个频道都共享同一套页面级范围条：全部 / 先锋杯 / 传奇杯 / 冠绝杯
- 切换页面级范围条只改变当前对象的杯赛上下文，不改变当前所在对象频道
- 搜索直达比赛、赛季、选手、战队
...
- 横轴是顶部对象入口：比赛 / 选手 / 战队
- 纵轴是页面级范围条：全部 / 先锋杯 / 传奇杯 / 冠绝杯
- 用户实际感知到的是“两层导航协同”，而不是看到一个额外的抽象概念栏目
- 页面级范围条就是页面上的二级导航展开，因此它不能被简化成普通筛选条；当前范围的标题、统计和右栏说明在导航条之外的正文或侧栏中同步体现；页面表现采用 community 风格的胶囊按钮行
- 比赛、选手、战队频道页的首屏固定顺序是顶部导航 -> 条件性范围 Banner（仅具体杯赛态，可选） -> 二级导航（页面级范围条） -> 主体
- 条件性范围 Banner 只允许存在 1 个；“全部”态不显示 Banner
...
- 并列工具层：搜索、我的、后台
- 第二层：仅存在于 /matches、/players、/teams 的页面级范围条，也就是二级导航，负责全部 / 先锋杯 / 传奇杯 / 冠绝杯
- 第二层以下的条件性 Banner：仅存在于 /matches、/players、/teams 的具体杯赛态，负责承接当前杯赛摘要，且必须位于二级导航下方
