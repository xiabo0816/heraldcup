...
| 内容槽位 | 左栏内容入口 4 组，分别对应比赛中心、人物档案、战队名册、身份链路；中栏平台定位与四模块主轴 1 组；右栏登录引导或身份进度 1 组 |
...
要生成的内容：比赛中心、人物档案、战队名册、身份链路等固定入口；中栏平台定位、主 CTA 与四模块主轴；右栏登录或身份进度
标题长度：10-18 字
描述长度：30-70 字
...
- 颜色：全部使用 neutral surface / outline / on-surface；先锋杯使用 pioneer primary-container + pioneer primary；传奇杯使用 legend primary-container + legend primary；冠绝杯使用 crown primary-container + crown primary
...
- 非当前态按钮继续使用当前主题 surface + border + text-secondary，不预铺低强度杯赛底色
