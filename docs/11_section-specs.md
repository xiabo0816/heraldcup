...
| 用途 | 在对象频道顶部固定展示全部、先锋杯、传奇杯、冠绝杯 4 个范围入口，并用当前范围的大块背景建立上下文 |
| 布局 | 左文中控右摘要 3 段范围舞台带 |
| 栅格占比 | 12 栏整宽 |
| 内容槽位 | 范围入口 4 个，当前态 1 组，范围说明 1 组，统计 0-3 项，状态胶囊 0-2 个 |
| 允许组件 | Tabs / Badge / SurfaceCard |
| 允许变体 | Tabs.style=segmented；Tabs.size=md；Tabs.tone=neutral|pioneer|legend|crown |
| 禁止 | 把范围条做成普通细筛选条；把“全部”做成任一杯赛色；当前态没有大块面承接 |
| 响应式 | xs 保持横向滚动，但顺序固定为全部 / 先锋杯 / 传奇杯 / 冠绝杯 |

补充规则：

- 当前态范围块面必须包住工作台起始线，不能只剩 Tab 底色
- “全部”范围固定使用中性色大块面；先锋杯、传奇杯、冠绝杯分别使用各自主题色系统
- 范围块面的形制、留白、圆角和分栏骨架四态保持一致，只替换主色和文案密度
- 范围说明最多 2 行，状态 Badge 最多 2 个，不允许形成二级营销 Banner

展示解剖：

- 左段：范围名、当前视图标题、1 到 2 行说明。示例：先锋杯 / 先锋杯选手池 / 当前仅展示先锋杯合资格选手
- 中段：4 个固定顺序范围块。当前态面积最大，非当前态为缩略块，但四项始终同时可见
- 右段：2 到 3 个统计项 + 0 到 2 个资格 Badge。统计优先展示当前目录计数、当前阶段或资格状态
- 底边：必须直接与下方主工作台共边，或通过 1 条连续 gutter 进入 8 + 4 主副栏，不单独断开成第二个说明区
...
- 全部：中性色舞台面 + 全站目录语义 + 可聚合跨杯对象
- 先锋杯：emerald / sky 色系 + 新晋与入门赛道语义
- 传奇杯：violet / amber 色系 + 成熟与秩序赛道语义
