...
- 对 /matches、/players、/teams 而言，若 currentScope=pioneer|legend|crown，可以输出“顶部导航 -> 胶囊二级导航 -> Banner -> 正文首块”，其中这个 Banner 内部允许使用 PageIntro，但不得再多起一个独立 PageIntro
- 首页默认全宽舞台
...
- 频道页若使用 PageIntro，只能把它落在唯一 Banner 内部，或作为正文首块自身的标题区；不能把它做成二级导航后的独立题头
- 频道页正文首块即使内部标题叫“比赛工作台”“选手目录”“战队目录”，它也必须作为正文区内部标题出现；不要因为这些名字像页名，就把它们交给 PageIntro 提前升格成频道页头
