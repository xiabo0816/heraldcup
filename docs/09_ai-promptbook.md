...
- 默认采用 14 栏栅格
- 顶部导航默认全宽
- 顶部导航默认拆成对象入口与工具入口两组；页面级范围条只允许出现在 /matches、/players、/teams 频道主区起始处，形式采用与 /community 一致的胶囊式子导航按钮行；颜色继续沿用原二级导航主题规则。“全部”态不出 Banner，具体杯赛态才允许保留 1 个对应 Banner。若该 Banner 承接人物、战队或赛程焦点内容，也必须直接复用这同一个 Banner 槽位，不得额外生成第二个正文 Banner
- 首页默认全宽舞台
