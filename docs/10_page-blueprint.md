...
- 首页不参与这套二级导航结构；任何 Footer 之前的区块都不能伪装为比赛/选手/战队页面专用的胶囊二级导航
...
- 第一期开赛相关页面必须把“顶部对象入口 + 二级导航”做成可感知结构：顶部对象入口负责对象维，顶栏下方的胶囊二级导航负责范围切换
- 胶囊二级导航不能退化成普通标签行；它作为二级导航必须保持简单、直接，核心任务只有范围切换；当前范围的标题、统计或说明放到正文首块或右侧关系卡里同步变化；“全部”态无 Banner，具体杯赛态才允许出现对应 Banner
- 当前范围一旦切到具体杯赛态，Blueprint 必须把二级导航以下从 Banner、PageIntro、工具条、主列表、右栏到回流区写成统一主题壳层；“全部”态再回到中性骨架
- 比赛、选手、战队三个频道之间横向跳转时，Blueprint 默认要求保留当前范围，否则这套二级导航只剩单页局部状态
- 胶囊二级导航固定采用 community 风格的胶囊二级导航行；Banner 不是导航本身，而是仅在具体杯赛态出现的附加区块
- 二级导航的视觉目标是“稳、清楚、次一级”：位于频道主区顶部、弱于顶部导航，但又明显强于正文里的普通筛选条或工具条
- 页面首屏必须做成连续壳层：顶部对象入口、二级导航、Banner、正文首块共享同一版心、同一纵向中心线和连续边界，不拆成彼此分离的多张头部卡片
...
顶部对象入口
-> 二级导航（胶囊按钮，4 个范围入口）
-> [仅 pioneer / legend / crown 出现] Banner
-> 主工作台或主目录（8 + 4 / 单列工作台）
-> 回流区
-> Footer
...
- 胶囊二级导航作为二级导航必须保持单层简单结构，不得拆成“信息区一层 + 按钮区一层”
- “全部”态时，二级导航下方直接进入工作台或目录，不能额外插中性色 Banner
- 具体杯赛态的 Banner 只能有 1 个，且必须作为频道页头部摘要存在，不得再插第二个题头块
- Banner 消失或出现时，主工作台整体上提或下移，但对象频道骨架不改变
- 切换到具体杯赛态时，不只 Banner 变色；主工作台、筛选条、右栏和回流区也要跟随进入同一主题壳层，但 layout、信息密度和对象频道骨架不改变
- 右侧侧栏顶线应与主工作台起始顶线对齐；有 Banner 时由二级导航 -> Banner -> 主工作台形成短过渡
...
- 具体杯赛态：当前杯赛按钮切到最强主题色；Banner 作为头部摘要存在于二级导航之后；Banner 与二级导航之间保持短过渡
...
- 一体化色面：顶部导航保持最深中性色；二级导航为浅一层稳态面；Banner 只在当前杯赛主色上做低一档雾面渐变，不把整块亮度抬到超过 Match Workspace 的主操作区
...
- 某杯态：二级导航当前按钮切到当前杯赛最强主题色；Banner（选手变体）作为头部摘要存在于二级导航之后；Banner 与二级导航之间保持 12 到 20 的短过渡
...
- 某杯态：当前按钮切到当前杯赛最强主题色；Banner（战队变体）作为头部摘要存在于二级导航之后；Banner 与二级导航之间保持 12 到 20 的短过渡
