...
- 中栏四模块标题：当前赛季与焦点对阵 / 认证选手与代表比赛 / 活跃阵容与当前归属 / 从登录到上场的下一步
...
| 区块顺序 | 区块名称 | 目标 | 布局模式 | 形状定义 | 拼接关系 | 主视觉权重 | 组件 | 主动作 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Left Rail Navigation | 把比赛中心、人物档案、战队名册、身份链路收进固定入口侧轨 | 左侧 3 栏内容入口侧轨 | 4 个纵向入口块，偏直角 | 与中栏共顶线；固定贴左侧起线；与中栏保持稳定 gutter | B | SurfaceCard / Link / Badge | 打开入口 |
| 2 | Center Content Stack | 承接首页主内容主体 | 中间 6 栏纵向主内容流 | 中栏顶部先讲平台定位与主 CTA，下方由比赛中心、人物档案、战队名册、身份链路 4 个明确模块顺排组成 | 与左右侧轨共顶线；在同一条中轴中连续展开；左右侧轨不得插入中栏主体 | A | SectionIntro / SurfaceCard / MatchCard / PlayerCard / TeamCard / Button | 查看内容详情 |
| 3 | Right Rail Identity | 右侧固定承担登录引导或登录后身份进度 | 右侧 3 栏身份轨 | 登录前为引导块，登录后为账号、认领、队伍与邀请摘要块；已成为选手时在下方追加个人入口块 | 与中栏共顶线；固定贴右侧起线；与中栏保持稳定 gutter | B | SurfaceCard / Button / Badge | 登录或进入我的主页 |
...
- 形状定义：4 个纵向入口块，分别对应比赛中心、人物档案、战队名册、身份链路；以 shape.none 到 shape.sm 为主，仅当前入口可提升到 shape.lg
...
交互：点击后跳转到对应页面或身份入口，不承担详情内容本身
禁止：不要把左栏写成资讯摘要，不要在左栏重复中栏详细数据
```

区块 2：Center Content Stack
- 目标：把首页主要内容稳定放在中间 6 栏；顶部先讲平台定位与主 CTA，再固定收敛为比赛中心、人物档案、战队名册、身份链路四个明确模块
- 布局模式：中间 6 栏纵向主内容流
- 栅格占比：lg 6，md 8，xs 14
- 形状定义：中栏顶部为唯一主重摘要块，下方按比赛中心、人物档案、战队名册、身份链路 4 段纵向分节；模块之间使用统一留缝和标题条，不额外再起第二舞台
...
- 响应式：xs 改为单列主内容流，仍保留“比赛中心 -> 人物档案 -> 战队名册 -> 身份链路”顺序
- Promptframe：

```text
区块：Center Content Stack
生成目标：用四个连续模块把首页中栏写成稳定内容主轴
模块顺序：比赛中心 -> 人物档案 -> 战队名册 -> 身份链路
...
语气：信息化、清楚、不过度宣传
禁止：不要伪造完整赛程，不要伪造选手全量数据，不要伪造完整战队阵容，不要把身份链路写成空泛口号
```
...
- 布局模式：右侧 3 栏身份轨
- 栅格占比：lg 3，md 4，xs 14
- 形状定义：登录前为单个引导主块；登录后至少包含 1 个账号与认领摘要块；已成为选手时再向下叠加 1 个队伍或邀请摘要块；以 shape.none 到 shape.sm 为主，局部单侧圆角只用于收口
- 拼接关系：固定贴右边界；与中栏保持连续 gutter；不同状态块纵向堆叠，但不能扩展成第二内容流
- 注意力权重：B
- 内容槽位：
	- 登录前：标题、登录收益说明、登录动作
	- 登录后未成为选手：账号摘要、认领进度、个人入口
	- 登录后已成为选手：账号摘要、队伍与邀请摘要、个人入口
...
已成为选手：在身份摘要下追加个人页或队伍入口
语气：直接、信息化、不要喊口号
禁止：不要把右栏写成广告区，不要用数据卡取代身份主位，不要在未成为选手时伪造队伍管理信息
```
...
- 颜色规格：全部使用 neutral surface + outline + on-surface；先锋杯使用 `#236D5C` 系列浅底与强调线；传奇杯使用 `#563880` 系列浅底与强调线；冠绝杯使用 `#853454` 系列浅底与强调线，必要时在实现层映射到 `cup-immortal` token
...
颜色规则：全部=neutral surface 体系；先锋杯=pioneer primary / primary-container；传奇杯=legend primary / primary-container；冠绝杯=crown primary / primary-container
