...
补充：双轴矩阵是第一期公开对象页的导航构成，不是额外露出的一个栏目名称或独立矩阵区块；页面设计应通过“顶部对象入口 + 页面级范围条 + 当前范围驱动的正文与侧栏”共同表达。
...
补充：比赛、选手、战队三个频道页采用双轴矩阵设计：顶部对象入口承载对象维，页面级范围条承载杯赛范围维；首页、阅读页、操作页、后台页不参与双轴矩阵。
...
修改时先判断你改的是哪一层：顶部对象入口、顶部工具入口、对象页范围次导航，还是首页内容入口侧轨；不要把四层写成同级。

双轴矩阵由对象导航、范围条、当前范围标题与统计、当前范围侧栏说明、跨对象范围连续性共同构成，不单独形成一个命名区块。
