...
补充：第一期公开对象页的导航构成就是“顶部对象入口 + 页面级范围条（Range Stage Band） + 对应杯赛上下文下的正文与侧栏”；不要再额外发明别的概念名或独立矩阵区块，页面级范围条本身就是这里的范围切换导航，不是普通筛选条。
...
补充：首页不参与这套二级导航结构；比赛、选手、战队频道的页面级范围条只属于对象频道，不进入首页。首页与其他页面都应在文档中定义 Footer 收束规则。

### 04_page-briefs-channel-and-reading.md

定义第一期频道页、阅读页、静态说明页的结构模板与任务优先级；第二期统一见 16_phase-two-features.md。

补充：比赛、选手、战队三个频道页采用“顶部对象入口 + 二级导航”的设计：顶部对象入口承载对象维，页面级范围条承载杯赛范围切换；首页、阅读页、操作页、后台页不参与这套二级导航结构。
...
这套二级导航结构由对象导航、页面级范围条（二级导航）、唯一的条件性范围 Banner 槽位、身份与资格侧栏、跨对象范围连续性共同构成，不需要再单独命名成别的概念区块。

页面级范围条就是比赛、选手、战队频道页的二级导航，不是单独的页内小组件，也不是普通筛选条；它现在采用与 community 频道一致的胶囊式子导航写法，出现在频道主区顶部、PageIntro 上方，固定承载 4 个范围入口。“全部”态不展示 Banner，只有切到具体杯赛态时才展示对应的范围 Banner。
...
比赛、选手、战队三个频道页的首屏层级必须固定读成“顶部导航 -> 条件性范围 Banner（仅具体杯赛态，可选） -> 二级导航（页面级范围条） -> 主工作台或主目录”；页面级范围条固定出现在频道主区入口处，不再做成紧贴顶部导航的整条范围带。

频道页首屏同一时刻只允许存在 1 个 Banner 槽位。所谓“条件性范围 Banner”和页面里承接人物、战队、赛程摘要的 Banner，其实是同一个东西：如果该页要展示焦点人物、推荐战队或当前杯赛摘要，就必须把这些内容写进这个唯一 Banner 里，不能在它下面再起第二个正文 Banner。
