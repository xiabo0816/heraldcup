  ...
  return (
    <Shell>
      <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <AdminContentPageForm matches={data.matches} topics={data.topics} />
        ...
        <section className="rounded-[28px] border border-white/10 bg-panel/80 p-6 shadow-glow backdrop-blur">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-400">
              在这里统一维护海报页、冠军页、快报和赛后战报，并把它们关联到具体比赛或话题。
            </p>
          </div>

          <div className="mt-6 space-y-4">
            {data.pages.map((page) => (
              <AdminContentPageRow key={page.id} page={page} matches={data.matches} topics={data.topics} />
            ))}
          </div>
        </section>
      </section>
